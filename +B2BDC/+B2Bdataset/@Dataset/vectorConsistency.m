function [vcReport, EXITFLAG] = vectorConsistency(dsObj,wY,wX, opt)
% vectorConsistency   Evaluates the vector consistency of a given dataset
% with given weights
%    [vcReport, EXITFLAG] = vectorConsistency(dsObj, wY, wX) calculates the vector consistency measure of the dataset. 
%    When wY and wX are 2-column numerical arrays,
%    they specify the weights associated with QOI and parameter uncertainty bounds, respectively. 
%    When they are charracter arrays, they specify certain types of weights:
%    'perc' for percentage weights; 'uwidth' for uncertanty width weights; 'unit' for unit
%    weights and 'null' for zero weights. In this case, the option variable is the default
%    B2BDC.Option.Option object. The vector consistency measure results are stored in
%    vcRport whereas EXITFLAG indicates the solution status of the inner bound from
%    fmincon.
%    
%    [vcReport, EXITFLAG] = vectorConsistency(dsObj, wY, wX, opt) calculates the vector
%    consistency measure of the dataset. When wY and wX are 2-column numerical arrays,
%    they specify the weights associated with QOI and parameter uncertainty bounds, respectively.
%    When they are charracter arrays, they specify certain types of weights:
%    'perc' for percentage weights; 'uwidth' for uncertanty width weights; 'unit' for unit
%    weights and 'null' for zero weights. In this case, the option variable is specified by opt.
%    The vector consistency measure results are stored in vcRport whereas EXITFLAG
%    indicates the solution status of the inner bound from fmincon
%
% The inputs are:
% --------------------------------------------------------------------------
...
% Ann model
if networktest(dsObj)
   [vcReport, EXITFLAG] = vectorConsistencyNN(dsObj,wY,wX, opt);
   return;
end
...
if ~strcmp(opt.Prediction,'inner')
...
if ~strcmp(opt.Prediction,'inner')
   ...
   z0 = [dsObj.Variables.makeLHSsample(nInit)'; zeros(dim-1-nVar,nInit)];
end
