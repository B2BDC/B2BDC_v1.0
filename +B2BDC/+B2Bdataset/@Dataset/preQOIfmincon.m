...
if nStart > 1
   ...
   b2bopt.SampleOption.SampleMethod = 'Gibbs';
   xSample = obj.collectSample(2*(nStart-1),[],b2bopt);
   b2bopt.SampleOption.StepInterval = n0;
   xStart = [xStart; xSample];
end
...
if linFlag
   for j = 1:nStart
      [xmin{j},ymin(j),minflag(j),~,ilam_min{j}] = fmincon(@funxmin_lin,xStart(2*j-1,:)',...
         A1,B1,Aeq,Beq,LB,UB,@neq,opt);
      [xmax{j},ymax(j),maxflag(j),~,ilam_max{j}] = fmincon(@funxmax_lin,xStart(2*j,:)',...
         A1,B1,Aeq,Beq,LB,UB,@neq,opt);
   end
else
   for j = 1:nStart
...
xOpt.max.dimension = [n_variable nMD];
   ...
   function [y,gy] = funxmin_lin(x)
      gy = zeros(n_variable+nMD,1);
      y = Q0(1,1)+f0*x(id0);
      gy(id0) = f0';
   end

   function [y,gy] = funxmax_lin(x)
      gy = zeros(n_variable+nMD,1);
      y = -Q0(1,1)-f0*x(id0);
      gy(id0) = -f0';
   end
   ...
   function [c,ceq,g,geq] = neq(x)
      c = zeros(2*n_units,1);
      g = zeros(n_variable+nMD,2*n_units);
      for i = 1:n_units
