...
bds(:,2) = bds(:,2) + abE;
name2 = obj.VarNames;
[idall,Qall,Nall,Dall] = obj.getQ_RQ_expansion(q);
id0 = idall{end};
Q0 = Qall{end};
N0 = Nall{end};
D0 = Dall{end};
...
d = bds(:,2)-bds(:,1);
nL = length(Qx) - n_variable -nMD;
if nL > 0
   tlb = LBD(:,1);
   tub = LBD(:,2);
   tdb = tub-tlb;
   L2 = L(n_variable+nMD+1:end,:);
end
cvx_begin sdp quiet
% cvx_solver sedumi
cvx_precision('best');
...
dual variable S
if ~isempty(N0)
   targetN = zeros(n_variable+nMD+1);
   targetD = zeros(n_variable+nMD+1);
   targetN([1;id0+1],[1;id0+1]) = N0;
   targetD([1;id0+1],[1;id0+1]) = D0;
   cvxconstr = targetN-tmin*targetD;
elseif ~isempty(Q0)
   targetMatrix1 = zeros(n_variable+nMD+1);
   targetMatrix2 = zeros(n_variable+nMD+1);
   targetMatrix2(1,1) = 1;
...
for i = 1:n_variable+nMD
   cvxconstr = cvxconstr + lamV(i)*Qx{i};
end
if nL > 0
   for i = 1:nL
      cvxconstr = cvxconstr + lamL(i) * Qx{n_variable+nMD+i};
   end
end
if n_extra ~= 0
   for k = 1:n_extra
...
maximize tmin
subject to
S: cvxconstr >= zeros(n_variable+nMD+1,n_variable+nMD+1);
lamEL >= 0;
lamEU >= 0;
lamV >= 0;
if nL > 0
