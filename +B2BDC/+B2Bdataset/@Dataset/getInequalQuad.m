...
if ~obj.ModelDiscrepancyFlag
   ...
   [idall,Qall] = getQ_RQ_expansion(obj);
   nlX = size(A0,1);
   n_variable = vars.Length;
   xbds = vars.calBound;
   LB = xbds(:,1);
   ...
   n_units = obj.Length;
   Qunits = cell(2*n_units,1);
   Qx = cell(n_variable+nMD+nlX,1);
   idRQ = false(n_units,1);
   for i = 1:n_units
      constrMatrix1 = zeros(n_variable+nMD+1);
      constrMatrix2 = zeros(n_variable+nMD+1);
