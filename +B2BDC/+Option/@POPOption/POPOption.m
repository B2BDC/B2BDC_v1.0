classdef POPOption < handle
   ...
   % ---------------------------------------------------------------------
   % relaxOrder:
   %   a numerical scalar specifies the relaxation order.
   % ---------------------------------------------------------------------
   % sparseSW:
   %   1 - use sparse relaxation
   %   0 - use dense relaxaton
   %   2 - use a smaller dense relaxation
   %   3 - use a smaller sparse relaxation
   % ---------------------------------------------------------------------
   % perturbation:
   %   a numerical scalar, typically very small such as 1e-5, that is added to
   %   the objective polynomial.
   % ---------------------------------------------------------------------
   % SDPsolver:
   %   a character array specifies the name of the SDP solver, 'sedumi', or
   %   'sdpa'.
   % ---------------------------------------------------------------------
   % printFileName:
   %   a numerical scalar or a character array for how to print the solution
   %   information: 0 - save no solution information; 1 - save solution information; a character
   %   array of the file name - print the solution in the file.
   % ---------------------------------------------------------------------
   % printLevel:
   %   a numerical array contains 2 elements, [a, b], where a and b specifies what
   %   information to be saved in the result and in the file, respectively: 0 - no information
   %   on the computational result; 1 - some information without an optimal solution; 2 -
   %   detailed solution information.
   % ---------------------------------------------------------------------
   % POPsolver:
   %   a character array specifies the solver used for computing the inner
   %   bound: 'active-set', 'trust-region-reflective', 'interior-point', or
   %   'lsqnonlin'
   % ---------------------------------------------------------------------
   % mex:
   %   1 - use C++ subroutines
   %   0 - not use C++ subroutines
   % ---------------------------------------------------------------------
   ...
   methods
      ...
      function set.printFileName(obj,num1)
         ...
         elseif ischar(num1) || isstring(num1)
            obj.printFileName = num1;
         else
            error('Invalid input property value')
         end
