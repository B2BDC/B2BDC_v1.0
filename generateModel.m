...
% (f) modelObj = generateModel(wCoef, networkSize, varList)
%  input:
%    wCoef      - A vector specifies the weights in the network
%  networkSize  - A vector specifies the size of each layer
%   varList     - A B2BDC.B2Bvariables.VariableList object contains variable
%                  informations (name, domain etc.)
%  output:
%    modelObj  - A B2BDC.B2Bmodels.NNModel object
...
elseif nin < 4
   ...
   elseif isvector(varargin{1}) && length(varargin{1}) == sum(([varList.Length varargin{2}]+1).*[varargin{2} 1])
      wVec = varargin{1};
      if size(wVec,1) == 1
         wVec = wVec';
      end
      ...
      cc = 0;
      ww = reshape(wVec(cc+1:cc+HL(1)*(nVar+1)),HL(1),nVar+1);
      cc = cc+HL(1)*(nVar+1);
      net.IW{1,1} = ww(:,1:end-1);
      net.b{1} = ww(:,end);
      for jj = 1:nL-1
         ww = reshape(wVec(cc+1:cc+HL(jj+1)*(HL(jj)+1)),HL(jj+1),HL(jj)+1);
         cc = cc+HL(jj+1)*(HL(jj)+1);
         net.LW{jj+1,jj} = ww(:,1:end-1);
         net.b{jj+1} = ww(:,end);
      end
      ww = wVec(cc+1:end);
      net.LW{nL+1,nL} = ww(1:end-1)';
      net.b{nL+1} = ww(end);
      modelObj = B2BDC.B2Bmodels.NNModel(net,varList); 
